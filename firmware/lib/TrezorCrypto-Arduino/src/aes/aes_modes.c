...
{   int nb = len >> AES_BLOCK_SIZE_P2;
    ...
    while(nb--)
    {
        if(aes_encrypt_trezor(ibuf, obuf, ctx) != EXIT_SUCCESS)
            return EXIT_FAILURE;
        ibuf += AES_BLOCK_SIZE;
        obuf += AES_BLOCK_SIZE;
    }
...
{   int nb = len >> AES_BLOCK_SIZE_P2;
    ...
    while(nb--)
    {
        if(aes_decrypt_trezor(ibuf, obuf, ctx) != EXIT_SUCCESS)
            return EXIT_FAILURE;
        ibuf += AES_BLOCK_SIZE;
        obuf += AES_BLOCK_SIZE;
    }
...
# ifdef FAST_BUFFER_OPERATIONS
    if(!ALIGN_OFFSET( ibuf, 4 ) && !ALIGN_OFFSET( iv, 4 ))
        while(nb--)
        {
            ...
            if(aes_encrypt_trezor(iv, iv, ctx) != EXIT_SUCCESS)
                return EXIT_FAILURE;
...
# endif
    ...
    while(cnt < len)
    {
        if(!b_pos && aes_encrypt_trezor(iv, iv, ctx) != EXIT_SUCCESS)
            return EXIT_FAILURE;
